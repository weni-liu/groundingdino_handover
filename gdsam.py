...
if __name__ == '__main__':
    ...
    parser.add_argument("--input_folder",default='D:\\Uploads\\Autolabeling_input')
                        ...
                        with open(txt_path, 'r', encoding='UTF-8') as f:
                            ...
                            for txt in lines:
                                ...
                                if len(txt.split(',')) == 5:
                                    ...
                                    out_dir = os.path.join(args.output_folder, 'Autolabeling_tmp', 'Label') #'D:\\Uploads\\autolabeling\\Label'
                                    label_name = 'defect'
                                    n_file = True
                                    if not os.path.isdir(out_dir):
                                        os.makedirs(out_dir)
                                ...
                                if len(objects) > 0:
                                    ...
                                    # pic_basepath = os.path.dirname(os.path.dirname(img_path))
                                    # img_path_s = os.path.join(pic_basepath, filename_element.text)
                                    if os.path.isfile(img_path):
                                        ...
                                        if len(maskes) > 0:
                                            ...
                                            write_json(image, filename_element.text, all_contours, out_dir, label_name)
                                        else:
                                            if n_file:
                                                write_non_json(filename_element.text, out_dir,label_name)
                                else:
                                    if n_file:
                                        write_non_json(img_path.split('\\')[-1], out_dir)
                        os.remove(txt_path)
